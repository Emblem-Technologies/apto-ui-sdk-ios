...
open class NavigationMenu: NSObject {
  ...
  init(viewController: UIViewController, uiConfiguration: UIConfig, menuListener: NavigationMenuListener) {
  ...
  func install() {
    viewController.installNavRightButton(UIImage.imageFromPodBundle("top_menu_default.png"),
                                         tintColor: self.uiConfiguration.tintColor,
                                         target: self, action: #selector(NavigationMenu.menuTapped))
  }
