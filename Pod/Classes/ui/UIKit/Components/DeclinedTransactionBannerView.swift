...
class DeclinedTransactionBannerView: UIView {
  ...
  init(declineReason: String, uiConfig: UIConfig) {
    ...
    self.titleLabel = ComponentCatalog.sectionTitleLabelWith(text: title, uiConfig: uiConfig)
    self.descriptionLabel = ComponentCatalog.mainItemRegularLabelWith(text: declineReason, multiline: true,
                                                                      uiConfig: uiConfig)
