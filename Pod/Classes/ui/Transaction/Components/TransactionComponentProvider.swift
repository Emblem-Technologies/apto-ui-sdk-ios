...
public protocol TransactionComponentProvider {
  ...
  /**
   `TransactionListView` component is in charge of getting the list of transactions from a `Card`, you can place this component anywhere in your app as long as
   you are logged in with our Core SDK. There's an options entity with extra parameters to configure the `TransactionListView`
  ...
  /**
   If you want to work with the `CreditCardView`` it's pretty simple to do so, we only need to call the `makeCardView()` method
   and a new `CreditCardView` component will be created. There are some options that you can pass to the component that can be found in
