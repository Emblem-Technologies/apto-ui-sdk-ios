...
class ShiftCardTransactionDetailsViewControllerTheme2: ShiftViewController, ShiftCardTransactionDetailsViewProtocol {
  ...
  private var previousShadowOpacity: Float = 0
  ...
  override func viewWillAppear(_ animated: Bool) {
    ...
    previousShadowOpacity = navigationController?.navigationBar.layer.shadowOpacity ?? 0
    navigationController?.navigationBar.layer.shadowOpacity = 0
  }

  override func viewWillDisappear(_ animated: Bool) {
    super.viewWillDisappear(animated)
    navigationController?.navigationBar.layer.shadowOpacity = previousShadowOpacity
  }
  ...
  func finishUpdates() {
    ...
    rows.append(contentsOf: declinedTransactionRow(viewModel))
  ...
  private func declinedTransactionRow(_ viewModel: ShiftCardTransactionDetailsViewModel) -> [FormRowView] {
    var rows: [FormRowView] = []
    if viewModel.transactionStatus.value == TransactionState.declined.description() {
      let reason = viewModel.declineReason.value
      let bannerView = DeclinedTransactionBannerView(declineReason: reason, uiConfig: uiConfiguration)
      let row = FormRowCustomView(view: bannerView)
      row.padding = uiConfiguration.formRowPadding
      rows.append(row)
    }
    return rows
  ...
  private func basicInfoSectionRowsFrom(_ viewModel: ShiftCardTransactionDetailsViewModel) -> [FormRowView] {
    ...
    addRowIfHasValue(rows: &rows,
                     ...
                     value: viewModel.fundingSourceName.value)
    return rows
  }
  ...
  private func detailsInfoSectionRowsFrom(_ viewModel: ShiftCardTransactionDetailsViewModel) -> [FormRowView] {
    ...
    if showDetails {
      ...
      addRowIfHasValue(rows: &rows,
                       ...
                       value: viewModel.transactionType.value)
    }
    return rows
  }
  ...
  private func adjustmentsSectionRowsFrom(_ viewModel: ShiftCardTransactionDetailsViewModel) -> [FormRowView] {
    ...
    if !viewModel.adjustments.isEmpty {
      for adjustment in viewModel.adjustments.array {
        ...
        case .capture:
          title = "transaction_details.adjustment.transfer_from.text".podLocalized()
        case .refund:
          title = "transaction_details.adjustment.transfer_to.text".podLocalized()
        default:
          title = ""
        }
        ...
        if let adjustmentExchangeRate = adjustment.exchangeRate,
          ...
          exchangeRate = "1 \(nativeCurrency) @ \(localCurrency) \(adjustmentExchangeRate.format(decimalPlaces: 2))"
        }
        var amountString: String?
        if let amount = adjustment.nativeAmount?.absText ?? adjustment.localAmount?.absText {
...
extension ShiftCardTransactionDetailsViewControllerTheme2: UIGestureRecognizerDelegate {
