...
class EnableCardAction {
  ...
  func run(callback: @escaping Result<Card, NSError>.Callback) {
    let okTitle = "card_settings.settings.confirm_unlock_card.ok_button".podLocalized()
    let cancelTitle = "card_settings.settings.confirm_unlock_card.cancel_button".podLocalized()
    UIAlertController.confirm(title: "card_settings.settings.confirm_unlock_card.title".podLocalized(),
      ...
      self.platform.unlockCard(self.card.accountId) { result in
